import './App.css';
import SwitchTools from './components/switchTools';
...
function App() {
  ...
  const captureCanvas = () => {
    ...
    fetch('http://localhost:7789/predict', requestOptions)
      .then(response => response.json())
      .then(data => alert("The predicted digit is: " + data['digit'] + ", with Accuracy: " + data['acc']));
  };
  ...
  return (
    <div>
      <div>
        <SwitchTools setToolType={setToolType} captureCanvas={captureCanvas} />
      </div>
      <canvas
        id="canvas"
        className="App"
